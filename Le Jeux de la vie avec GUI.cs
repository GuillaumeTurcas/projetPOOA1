...
namespace Le_Jeu_de_la_vie_GUI
{
    class Program
    {
        ...
        //Afficher la grille (non utilisé dans le dans le cas de l'utilisation du GUI).........................................................
            ...
            else
            {
                for (int i = 0; i < L; i++)
                {
                    ...
                    while (j != C) 
                    {
                        ...
                        if (Grille[i, j] == 5) { Console.Write("¤ "); } //Cellule Vivante Population 2
        ...
        static int Taille(int[,] Grille, int Popul) //Compter le nombre de cellule vivante
        {
            ...
            for (int i = 0; i < L; i++)
            {
                for (int j = 0; j < C; j++)
                {
                    ...
                    if (Popul == 2 && Grille [i,j] == 5) //Pour compter la population 2
                    {
                        taille++;
                    }
                ...
                for (int j = 0; j < C; j++)
                {
                    ...
                    if (Alea < T * 10)
                    {
                        if (Visualisation == 4 || Visualisation == 3) //Cas si l'utilisateur veut deux pupulations sur la grille
                        {
                            ...
                            if (Repart == 0)
                            {
                                Grille[i, j] = 5; //créé une cellule de la génération 1
                                Repart1++;
                            }
                            else
                            {
        ...
        static int CompterAutourCellule(int[,] GrilleProv, int i, int j, int C, int L, int Rang, int Popul) //Compte le nombre de cellule vivante autour de la cellule sélectionnée
        {
            ...
            for (int x = i - Rang; x < i + Rang + 1; x++)
            {
                for (int y = j - Rang; y < j + Rang + 1; y++)
                {
                    ...
                    if (VOM == 5 && Popul == 2 && (x != i || y != j)) { Nombre++; } //Cas Population 2
                    x = tempX;
                    y = tempY;
                }
            }
